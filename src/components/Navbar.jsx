...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <Link
              to="/contact"
              className="hidden lg:block relative group overflow-hidden rounded-xl px-6 py-3 font-semibold text-white transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-slate-950"
            >
            ...
            >
              <div className="w-5 h-4 flex flex-col justify-between">
                <span className={`block h-0.5 w-full bg-slate-200 rounded-full transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-[7px]' : ''}`}></span>
                <span className={`block h-0.5 w-full bg-cyan-400 rounded-full transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`}></span>
                <span className={`block h-0.5 w-full bg-slate-200 rounded-full transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-[7px]' : ''}`}></span>
              </div>
            </button>
          </div>
      ...
      >
        <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-xl" onClick={() => setIsMenuOpen(false)}>
        ...
        <div className={`relative h-full flex flex-col pt-24 pb-8 px-6 transition-all duration-500 delay-100 ${isMenuOpen ? 'translate-y-0' : '-translate-y-8'}`}>
          <div className="flex-1 flex flex-col gap-2">
            {navLinks.map((link, index) => {
              ...
              return (
                ...
                >
                  <span className={`text-lg font-medium ${isActive ? 'text-cyan-400' : 'text-slate-200'}`}>
                    {link.name}
                  </span>
                  {isActive && (
          ...
          {/* Mobile Get Started → /contact */}
          <Link
            to="/contact"
            className="relative group overflow-hidden rounded-2xl px-8 py-4 font-semibold text-white transition-all duration-300 hover:scale-[1.02] shadow-xl"
          >
          ...
          <div className="mt-8 pt-8 border-t border-blue-500/20">
            <p className="text-xs font-semibold text-cyan-400 tracking-wider mb-3">CONTACT US</p>
            <a href="mailto:max@1252.company" className="block text-sm text-slate-300 hover:text-cyan-400 transition-colors mb-2">max@1252.company</a>
            <a href="tel:+6590026490" className="block text-sm text-slate-300 hover:text-cyan-400 transition-colors">+65 9002 6490</a>
          </div>
        </div>
      </div>
    </>
